...
class mass_media_two(dynamics):

    def __init__(self, net, parameters, options, nach, vec_B, T):
    ...
    def node_dyn(self, i):
        ...
        if ( dice < self.B):
            ...
            vec_simM2 = self.simMassMedia2(n1)
            Pm = P=np.sum(vec_simM2)/self.F   
            # Interaction probability
        
            if(random.random() < Pm):
            
                atrs_ch=select_atr(vec_simM2, self.n_ach, self.F)  
                # Obtain the list of indeces of attributes to change
                
                for atr in atrs_ch:
                # Change the attributes of the node n1 as many times n_ach said    
                    self.net_state.nodes[n1].vec_Param[atr]=self.media2[atr]
    ...
    def simMassMedia2(self, n1):
        ...
        for i in range(F):
            if self.net_state.nodes[n1].vec_Param[i]==self.media2[i]:
                vec_s.append(1)
            else:
                vec_s.append(0)
        return vec_s
    ...
    def save_color_media(self, file_name):
        ...
        gr3.add_node("Media2", pos=(-1,1), node_size=800 )
        ...
        position2 = nx.get_node_attributes(gr3,'pos')
        nx.draw(gr2, pos= position,
        node_color= colormap(self.color_media()) , with_labels=True, node_size=1500, font_color='black')
        nx.draw(gr3, pos= position2,
        node_color= colormap(self.color_media2()) , with_labels=True, node_size=1500, font_color='black')
    ...
    def color_media2(self):
        base = self.Q
        length = self.F
        step = 1/(base**length)
        val_map = lambda i:  step*i 
        value = val_map(self.n_base_mass2())
        return value
